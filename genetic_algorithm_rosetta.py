...
class genetic_algo:
    ...
    def mutate(self, ind, lista_fixed):
        ...
        print(f"Index allowed to mutate: {inds_to_mut}")
        if self.gene_type == 'discrete':
        ...
        if self.gene_type == 'continuous':
            position = int(np.random.choice(inds_to_mut, 1))
            gene = str(np.random.choice(self.gene_values,1))[2]
            ind[position] = gene
        ...
        if self.selection_method == 'tournament':
            ...
            for trymut in range(len(offspring)):
                if uniform(low=0, high=1) < self.mutation_rate:
                    offspring[trymut] = self.mutate(offspring[trymut], self.lista_fixed)
    ...
    def write_out(self, t):
        with open(self.file_name, 'a') as file:
            # Iterate through the list of lists and write each list as a line in the file
            for indiv_index in range(len(self.population)):
                # Convert the list to a string by joining its elements with commas
