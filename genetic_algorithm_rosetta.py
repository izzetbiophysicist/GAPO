...
class genetic_algo:
    def __init__(self, pose, opt_direction, gene_values, gene_type, vector_size, pop_size, mutation_rate, segment_fluctuation, apt_function, selection_method, convergence_threshold, n_cycles, n_survivors, tournament_size=0, initial_population=[]):
    ...
    def calculate_scores(self, population, pre_calc=[]):
        ...
        #### Iterate over population and fill self.scores
            scores = [self.apt_function(population[x], self.pose) for x in range(len(population))]
        else:
            scores = list(pre_calc) 
            scores_to_append = [self.apt_function(population[x], self.pose) for x in range(len(population)) if x >= len(pre_calc)]
